        ...
        <el-card>
            <template #header>
                <div class="my_refresh">
                    ...
                    <el-row>
                        <el-button size="small" type="primary" @click="onRefresh()" style="margin-left: 10px" :loading="loading.button">刷新</el-button>
                    </el-row>
                </div>
            </template>
            <div v-loading="loading.data">
                <div v-for="(item, index) in options" :key="index" >
                    ...
                    <div class="value">
                        <span v-if="! item.edit"><el-text>{{ item.value }}</el-text></span>
    ...
    methods: {
        ...
        loadUpdateOptions: function (data) {
            ...
            UpdateOptions(data).then(() => {
                ...
                let msg = err.response.data.metadata.message
                this.$notify({title: '更新失败', duration: 5000, message: msg, type: 'warning', })
                this.onRefresh()
            })
        },
        statusSwitch(val) {
