<template>
    <el-card style="margin-bottom: 20px;">
        ...
        <el-descriptions :column=2 >
            <el-descriptions-item width="50%" label="策略名称"><el-tag>{{ policyInfo.name}}</el-tag></el-descriptions-item>
            <el-descriptions-item label="策略ID"><el-tag>{{ policyInfo.id }}</el-tag></el-descriptions-item>
            <el-descriptions-item label="描述"><el-tag>{{ policyInfo.description }}</el-tag></el-descriptions-item>
            <el-descriptions-item label="创建时间"><el-tag>{{ formatDate(policyInfo.create_time )}}</el-tag></el-descriptions-item>
        </el-descriptions>
    </el-card>
    <el-card>
...
export default {
    ...
    data() {
        return {
            ...
            activeName: 'first',
            ChoosingRoles: [],
            Statement: [{ Action: ["uias:roles:createRoles", "uias:roles:createRoles", "uias:roles:createRoles"], Effect: "Allow" },
            {Action: ["uias:roles:createRoles","uias:roles:createRoles","uias:roles:createRoles"], Effect: "Allow"}]
        }
    },
    methods: {
        formatDate(time) {
        ...
        loadGetPoliciesInfo: async function (policy_id) {
            try {
                ...
                this.Statement = res.payload.policy.policy.Statement
                this.loading = false
            } catch (err) {
                this.tableData = []
                this.loading = false
        ...
        loadSelectRolesFromPolicy: function (policy_id) {
            ...
            SelectRolesFromPolicy(paths).then(res => {
